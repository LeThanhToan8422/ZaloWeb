...
const ListChat = ({ handleChangeChat, chats, userId, messageFinal }) => {
  ...
  useEffect(() => {
    if(messageFinal !== ""){
      let findMessage = chats.find(c => (c.sender === messageFinal.sender.id || c.sender === messageFinal.receiver.id) && (c.receiver === messageFinal.sender.id || c.receiver === messageFinal.receiver.id));
  ...
  let handleChangeSearch = async(value) => {
    let datas = await axios.get(`http://localhost:8080/relationship/get-friends-of-${userId}/${value}`)
